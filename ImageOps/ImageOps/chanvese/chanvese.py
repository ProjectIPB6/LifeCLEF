...
def chanvese_2(img_path, mask_min_x, mask_max_x, mask_min_y, mask_max_y, max_its=200, alpha=0.2):
    img = nd.imread(img_path)
    mask = np.zeros(img.shape)
    mask[mask_min_y:mask_max_y, mask_min_x:mask_max_x] = 1
    return chanvese(img, mask, max_its=max_its, display=False, alpha=alpha)


if __name__ == "__main__":
    img = nd.imread('brain.png', flatten=True)
    mask = np.zeros(img.shape)
    mask[50:500, 250:600] = 1
    
    chanvese(img, mask, max_its=200, display=False, alpha=0.2)
